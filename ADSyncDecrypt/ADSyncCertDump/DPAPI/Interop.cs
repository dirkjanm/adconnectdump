...
namespace SharpDPAPI
{
    public class Interop
    {
        ...
        public struct BCRYPT_RSAKEY_BLOB {
            ...
            public uint PublicExponentLen;
            public uint ModulusLen;
            public uint Prime1Len;
            public uint Prime2Len;
        }

        public struct BCRYPT_RSAPUBLIC_BLOB {
            public byte[] PublicExponent;
            public byte[] Modulus;

            public BCRYPT_RSAPUBLIC_BLOB(byte[] blob) {
                var reader = new BinaryReader(new MemoryStream(blob));
                var header = reader.ReadStruct<BCRYPT_RSAKEY_BLOB>();
                
                if(header.Magic != 0x31415352) { //RSA1
                    throw new FormatException("BLOB doesn't look like an RSA public key");
                }

                PublicExponent = reader.ReadBytes((int)header.PublicExponentLen);
                Modulus = reader.ReadBytes((int)header.ModulusLen);
            }
        }
