...
namespace SharpDPAPI
{
    public class Crypto
    {
        ...
        public static byte[] DecryptBlob(byte[] ciphertext, byte[] key, int algCrypt, PaddingMode padding = PaddingMode.PKCS7)
        {
        ...
        public static byte[] DeriveKey(byte[] keyBytes, byte[] saltBytes, int algHash, byte[] entropy = null)
        {
            ...
            if(keyBytes.Length > 20) {
                keyBytes = new SHA1Managed().ComputeHash(keyBytes);
            }
        ...
        public static byte[] AESDecrypt(byte[] key, byte[] IV, byte[] data, PaddingMode paddingMode = PaddingMode.PKCS7)
        {
        ...
        public static byte[] PinToEntropy(byte[] hexPin, int rounds, byte[] salt) {
            var pbkdf2 = new Pbkdf2(HMAC.Create("HMACSHA256"), hexPin, salt, rounds);
            var inter = pbkdf2.GetBytes(32, "sha256");
            var pbkdf2Output = Encoding.Unicode.GetBytes(Helpers.ByteArrayToString(inter, true));
            return SHA512.Create().ComputeHash(pbkdf2Output);

        }

        public static byte[] PinToEntropy(string pin, int rounds, byte[] salt) {   
            byte[] pbkdf2Input = Encoding.Unicode.GetBytes(
                Helpers.ByteArrayToString(Encoding.ASCII.GetBytes(pin), true));  
            return PinToEntropy(pbkdf2Input, rounds, salt);
